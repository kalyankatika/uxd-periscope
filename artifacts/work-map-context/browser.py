...
if len(args) > 1 and args[0] in {"click", "fill", "check", "uncheck", "select"}:
    ...
    scroll = subprocess.run([BIN, "--session", "work-map-context", "scrollintoview", args[1]], text=True, capture_output=True, timeout=45)
...
result = subprocess.run([BIN, "--session", "work-map-context", *args], text=True, capture_output=True, timeout=45)
