...
if len(args) > 1 and args[0] in {"click", "fill", "check", "uncheck", "select"}:
    ...
    scroll = subprocess.run([BIN, "--session", "periscope-people", "scrollintoview", args[1]], text=True, capture_output=True)
...
result = subprocess.run([BIN, "--session", "periscope-people", *args], text=True, capture_output=True)
